...
# ox% table (PDS or piqRUN(SF))
df_257 = pd.read_csv(inpt2)

# PMC
lst_PMC = df_257.iloc[:, 0]

# obtain elemental oxides from the oxide table
lst_sodium_ox = df_257.iloc[:, 1]
lst_magnesium_ox = df_257.iloc[:, 2]
lst_aluminum_ox = df_257.iloc[:, 3]
lst_silicon_ox = df_257.iloc[:, 4]
lst_phosphorus_ox = df_257.iloc[:, 5]
lst_sulfur_ox = df_257.iloc[:, 6]
lst_chlorine_ox = df_257.iloc[:, 7]
lst_potassium_ox = df_257.iloc[:, 8]
lst_calcium_ox = df_257.iloc[:, 9]
lst_titanium_ox = df_257.iloc[:, 10]
lst_chromium_ox = df_257.iloc[:, 11]
lst_manganese_ox = df_257.iloc[:, 12]
lst_iron_ox = df_257.iloc[:, 13]
lst_nickel_ox = df_257.iloc[:, 14]
lst_zinc_ox = df_257.iloc[:, 15]
lst_bromine_ox = df_257.iloc[:, 16]
...
iron_magnesium_moles = [float(iron) + float(magnesium) for iron, magnesium, calcium in zip(iron_moles, magnesium_moles)]
